...
closure_adjustments <- list(
  "screwtop" = c(scale = 0.8, shape = 0.9),
  "cork" = c(scale = 1.5, shape = 1.2)
)
...
priors <- c(
  prior(normal(0, 1), class = "b"),          # Slope for x
  prior(normal(0, 1), class = "Intercept") # Intercept
  #prior(exponential(1), class = "sigma")     # Prior for scale parameter of the gamma distribution
)
